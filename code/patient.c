...
void patient(char *nom)
{
    ...
    patient_t *patient = (void *) vac + sizeof(vaccinodrome_t);
    ...
    for(id_patient = 0; id_patient < (vac->n + vac->m); id_patient++) // pat_count ???? n+m ???
    {
        if(patient->status == LIBRE)
        {
            patient->status = OCCUPE;
            strncpy(patient->nom, nom, MAX_NOMSEM + 1);
    ...
    adebug (0, "id = %d", id_patient);
    asem_wait(&(patient->s_patient)); // attend que medecin cherche patient
    // vac->salle_count--;
    asem_post(&(vac->salle_attente)); // libere place sale d'attente

    int id_medecin = patient->id_medecin;
    printf("%d %d\n", id_patient, id_medecin);

    asem_post(&(patient->s_medecin)); // rejoint médecin dans la salle d'attente // necessaire ?

    asem_wait(&(patient->s_patient)); // attend fin vaccination

    vac->pat_count--;
    patient->status = LIBRE;
    fprintf(stdout, "P\n");
