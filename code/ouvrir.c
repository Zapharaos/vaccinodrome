...
void ouvrir(int n, int m, int t)
{
    ...
    asem_init (&(vac->is_in_salle), "is_in_salle", 1, 0); // salle d'attente du vaccinodrome côté médecin
    ...
    // asem_init (&(vac->medecins), "medecin", 1, 1);
    ...
    for(int i=0; i < (n+m); i++)
    {
        ...
        asem_init(&(patient->s_patient), "s_patient", 1, 0);
        asem_init(&(patient->s_medecin), "s_medecin", 1, 0);
        patient = (void *) patient + sizeof(patient_t);
    }
}
